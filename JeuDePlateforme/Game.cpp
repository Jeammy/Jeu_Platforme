...
void Game::update()
{
	...
	if (testCollisionBoxes(*player, *exit) && can_exit == true)
	{
		player->retriveLife();
	}
...
//SSPEICHERT
bool Game::testCollisionBoxes(Sprite spr1, Sprite spr2)
{
...
void Game::BlockBuilder(const char levelPath[], const char BlockTPath[])
{
	...
	for (int i = 0; i < (HAUTEUR / 32); i++)
	{
		for (int j = 0; j < (LARGEUR / 32); j++)
		{
			...
			if (strLevel[i][j] == '$')
			{
				SpawnerCollectible * spawnerC = new SpawnerDiamond();
				Collectible * collectible = spawnerC->FabriqueCollectible(j * 32, i * 32, &mainWin);
			...
			if (strLevel[i][j] == 'E')
			{
				exit = new Exit(j * 32, i * 32, &mainWin);
				exit->chargerTextures("Content\\Sprites\\exit.png");
				exit->setOrigin(exit->getTexture()->getSize().x / 2, exit->getTexture()->getSize().y / 2);
			}
		}
	}
}
...
bool Game::VerifierCollisionBetweenPlayerAndBlock()
{
	...
	for (iter = blockList.begin(); iter != blockList.end(); iter++)
	{
		...
		{
			if (testCollisionBoxes(*player, *iter) && player->getPosition().y - player->getTexture()->getSize().y/2 < iter->getPosition().y)
			{
...
bool Game::VerifierCollisionBetweenPlayerAndCollectible()
{
	...
	for (iterCollectible = collectibleList.begin(); iterCollectible < collectibleList.end(); iterCollectible++)
	{
		...
		{
			if (testCollisionBoxes(*player, *(*iterCollectible)))
			{
...
bool Game::VerifierCollisionBetweenPlayerAndEnnemy()
{
	...
	for (iterEnnemy = ennemyList.begin(); iterEnnemy != ennemyList.end(); iterEnnemy++)
	{
		...
		if (testCollisionBoxes(*player, *(*iterEnnemy)))
		{
			...
			else if (typeid(*(*iterEnnemy)) == typeid(Spike))
			{
				player->retriveLife();
			}
			return true;
...
void Game::VerifierCollisionBetweenEnnemyAndBlock()
{
	...
	for (iterBlock = blockList.begin(); iterBlock != blockList.end(); iterBlock++)
	{
		for (iterEnnemy = ennemyList.begin(); iterEnnemy != ennemyList.end(); iterEnnemy++)
		{
			...
			{
				if (testCollisionBoxes(*(*iterEnnemy), *iterBlock) && player->getPosition().y < (*iterEnnemy)->getPosition().y && distance < player->getTexture()->getSize().y / 2 + (*iterEnnemy)->getTexture()->getSize().y / 2)
				{
					(*iterEnnemy)->stop();
				}
